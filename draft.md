Kibo Script v0.5-draft

Language Specification

## 1. 目的

Kibo Script は、小型デバイス向けの bounded かつ inspectable な制御スクリプト言語である。対象は LED、PWM、ボタン、ADC、IMU、サーボ、モーター、OLED、UART / USB serial を中心とする。
...
Kibo Script は中括弧によるブロックを基本とする。インデントは整形規約として重視するが、構文上の境界は中括弧で定まる。
...
実行環境は runtime world を 1 つ持つ。runtime world は、デバイス、`ref`、`var`、`const`、`state`、`task` を保持する登録済みの世界である。

`script` は runtime world へ宣言を登録するための入力単位であり、実行単位ではない。利用者に見える基本実行単位は `task` である。

各 task は cooperative に実行される。

### 7.1.1 runtime world

runtime world は次の registry を持つ。

| registry | 内容 |
| --- | --- |
| device world | `led#0`、`display#0`、`button#0` などの外部対象 |
| ref registry | `ref led = led#0` のような immutable alias |
| var registry | `var` の現在値と writer 情報 |
| const registry | `const` の値 |
| task registry | 登録済み task と実行状態 |
| state machine registry | 登録済み状態機械、active leaf、elapsed |

登録入力は内部的には `compiled unit` として扱ってよい。ただし、通常の利用者に見える一覧や操作は task、ref、var、state machine を中心にする。
...
| 操作 | 意味 |
| --- | --- |
| add / register | 現在の runtime world に宣言を追加する |
| reset and load | runtime world を初期化してから宣言を登録する |

interactive 利用では add / register を基本とする。教材やデモでは reset and load を用いて、既存の task や状態を明示的に破棄してよい。

### 7.1.3 追加登録時の名前解決

後から登録する task や state machine は、すでに runtime world に登録済みの `ref`、`var`、`const`、`state` を ambient symbols として参照できる。
...
上の `task blink` は、既に登録済みの `ref led` を参照して登録される。

compile / semantic check は「新規入力」と「既存 runtime world metadata」を合わせた名前空間を前提に行う。

登録処理は atomic とする。診断や競合がある場合、runtime world は部分更新されない。例えば `ref` だけ登録されて task 登録に失敗する状態は作らない。
...
`var` は runtime world に登録される共有状態であり、読み取りはどの task からも許可する。一方、`set` による書き込み権限は task に帰属する。

初期仕様では writer は明示構文ではなく、compile / semantic check 時に暗黙に決定する。

```text
var requested_mode = "sweep"

task toggle_mode on button#0.pressed {
  set requested_mode = "pulse"
}
```

この場合、`requested_mode` の writer は `toggle_mode` である。別 task が同じ `var` に `set` する場合は、状態排他性により同時 writer にならないことを証明できる場合を除き、診断にする。

明示的な所有宣言（例: `task toggle owns requested_mode ...`）は、本版では導入せず今後の検討項目とする。

### 9.1 登録名の重複

runtime world へ追加登録する場合、名前の衝突は次のように扱う。

| 対象 | 初期方針 |
| --- | --- |
| 同名 task | 拒否 |
| 同名 ref | 同じ宛先なら冪等、違う宛先なら拒否 |
| 同名 var | 拒否 |
| 同名 const | 拒否 |
| 同名 state machine | 拒否 |

同名 `const` の同一値再登録を許すかは将来検討とする。初期仕様では、利用者が状態を追いやすいよう重複拒否を基本にする。

### 9.2 状態所属 task と `var` 書き込み
...
### 9.4 device effect の競合

`do` は外界への作用要求であり、`var` の `set` と同じ所有規則にはしない。

初期仕様では、同じ device への `do` は複数 task から許可する。

```text
task led_on on button#0.pressed {
  do led#0.on()
}
...
ただし、`toggle()` のような順序依存の作用は、複数 task から使うと結果を追いにくい。必要に応じて 1 つの driver task に寄せ、他の task は `var` や将来の event / signal で要求を渡すことを推奨する。

device 種別ごとの所有規則は段階的に強める。

| device | 初期方針 | 将来方針 |
| --- | --- | --- |
| `serial` | 複数 task から許可 | 必要なら出力整形のため writer warning |
| `display` | 複数 task から許可 | 描画の一貫性が必要なら owner 推奨 |
| `led` | 複数 task から許可 | `toggle()` 多用時は warning |
| `pwm` | 複数 task から許可 | owner warning または owner 必須を検討 |
| `motor` | 複数 task から許可 | owner warning または owner 必須を検討 |
| `servo` | 複数 task から許可 | owner warning または owner 必須を検討 |

### 9.5 drop と所有権

`drop task` は、その task が所有していた `var` writer 権限を解放する。ただし、`var` 自体は削除しない。

`drop ref`、`drop var`、`drop state` は、依存する task や state machine が存在する場合は拒否する。依存関係を無視して削除すると runtime world の名前解決と所有規則が壊れるためである。

source text や compiled unit の provenance は内部 metadata として保持してよい。ただし通常 UI では、`list tasks` を主役にし、必要に応じて `list refs`、`list vars`、`list states` を併用する。
...
interactive terminal は、script とは別の閉じた世界ではない。同じ runtime world を inspect / 操作する入口である。
...
上の `led.info` は、直前の入力 text に閉じた名前解決ではなく、runtime world に登録済みの `ref led` を解決している。
...
device 直書きと registered `ref` は同じ resolver で扱う。
...
| `read` の意味 | 常に同じ |
| 属性参照 | 常に同じ |
| 名前解決 | runtime world の registry を参照する |
| `var` 書き換え | 通常 command では導入しない |

terminal から通常の `set var = ...` を許可すると、task に帰属する writer ownership を壊す。必要な場合は、将来 `debug set` や `force set` のような明示的な debug command として検討する。

## 19. interactive な対象選択

通常の宣言は静的束縛を基本にする。対話利用でも、登録された `ref` は runtime world の ref registry に置かれる。

対話環境では `bind` のような実行環境操作を持ってよい。ただし、それは別文法の名前空間を作るものではなく、ref registry へ alias を追加する操作として扱う。

| 場面 | 方針 |
| --- | --- |
| 通常宣言 | `ref knob = adc#0` |
| 対話利用 | `bind probe = adc#3` のような実行環境操作を ref registry への登録として扱う |
...
| `list refs` | 登録済み ref 一覧 |
| `list vars` | 登録済み var の現在値と writer |
| `list states` | 状態機械の active leaf と elapsed |
...
`list tasks` は task 中心の世界観を保つための主コマンドである。`list refs`、`list vars`、`list states` は、同じ runtime world の周辺 registry を確認する補助コマンドである。
...
> list vars
scan_x          16       writer: oled_sweep_dashboard
requested_mode  "pulse"  writer: toggle_oled_mode

> list states
oled  active: oled.Pulse  elapsed: 120ms
```

登録済み `ref` は terminal の `read`、`do`、`.info` で device 直書きと同じように使える。
...
登録処理は atomic である。入力に複数の宣言が含まれ、その一部に診断が出た場合、どの宣言も runtime world へ追加しない。
...
| explicit ownership | `task name owns var_name ...` のような明示 writer 構文を導入するか検討する |
| device ownership policy | `pwm`、`motor`、`servo` などの作用先に owner warning / owner 必須を導入するか検討する |
| `inspect` command | `inspect name` で ref / var / task / state を横断表示する command を検討する |
| runtime reset / drop policy | 依存関係を持つ registry 要素の drop、reset、provenance 表示を整理する |
...
12. runtime world は task / ref / var / state machine registry を持ち、script は登録入力として扱う
13. interactive 利用では、自動 echo、info、list tasks / refs / vars / states を実行環境側へ置く
14. terminal は同じ runtime world を inspect / 操作し、登録済み ref を device 参照として解決できる
15. モード切り替えは持たず、言語本体の意味を一貫させる
