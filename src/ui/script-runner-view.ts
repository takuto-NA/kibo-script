...
export function build_kibo_package_serial_line(package_text: string): string {
  const package_root = JSON.parse(package_text) as unknown;
  const minified_package_text = JSON.stringify(package_root);
