...
### `examples/pico-runtime-samples`（5 本）の minified UTF-8 と `KIBO_PKG` 1 行長（再現コマンド付き）

計測日: **2026-05-04**。minified canonical JSON の UTF-8 byte 数と、`KIBO_PKG schema=1 bytes=<n> crc32=<8hex> b64=<payload>` 形式における **1 行の文字数**（改行除く）を `npx tsx` で算出した。

再現（リポジトリルート）:

```bash
npx tsx -e "import { readFileSync } from 'node:fs'; import { join } from 'node:path'; import { Buffer } from 'node:buffer'; import { compileScript } from './src/compiler/compile-script.ts'; import { buildPicoRuntimePackageCanonicalJsonTextFromCompiledProgramWithInferenceOrThrow } from './src/runtime-conformance/build-pico-runtime-package-from-runtime-ir-contract.ts'; const root=process.cwd(); const m=JSON.parse(readFileSync(join(root,'examples/pico-runtime-samples/samples.json'),'utf-8')); for (const s of m.samples){ const src=readFileSync(join(root,'examples/pico-runtime-samples',s.sourceFile),'utf-8').replace(/\r\n/g,'\n'); const c=compileScript(src,s.sourceFile); if(!c.ok)throw 0; const pkg=buildPicoRuntimePackageCanonicalJsonTextFromCompiledProgramWithInferenceOrThrow({compiledProgram:c.program,scriptVarNamesToIncludeInTraceOverride:s.traceVars}); const bytes=Buffer.byteLength(pkg,'utf8'); const b64=Buffer.from(pkg,'utf8').toString('base64'); const line=('KIBO_PKG schema=1 bytes='+bytes+' crc32=00000000 b64='+b64).length; console.log(s.name+'\t'+bytes+'\t'+line); }"
```

| sample | minified UTF-8 bytes | `KIBO_PKG` 1 行文字数（概算） |
| --- | ---: | ---: |
| `led-heartbeat` | 1384 | 1896 |
| `circle-sweep` | 3343 | 4508 |
| `two-circle-chase` | 5617 | 7540 |
...
MVP 範囲では JSON のまま **Go**（2026-05-04 追記: 5 サンプル実測でも decode 上限の半分以下）。上限接近の監視だけ継続する。
