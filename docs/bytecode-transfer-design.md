...
計測日: **2026-05-05**。minified canonical JSON の UTF-8 byte 数と、`KIBO_PKG schema=1 bytes=<n> crc32=<8hex> b64=<payload>` 形式における **1 行の文字数**（改行除く）を実機 acceptance の preflight で確認した。
