...
| Pico heap（staging / parse / live） | 実機で `diag=ram_probe`（[`probe_pico_runtime_package_ram_capacity.py`](../scripts/pico/runtime_vertical_slice/tools/probe_pico_runtime_package_ram_capacity.py)、`run_mvp_hardware_acceptance.py --profile ram`）。記録枠は [`docs/runtime-pico-handoff.md`](runtime-pico-handoff.md) の「5a) RAM 容量計測」 | `12288` 引き上げ判断は計測表を埋めてから |
